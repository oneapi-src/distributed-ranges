 ...
 * beginning at out.
 * \param policy use `dr::sp::par_unseq` here only
...
auto transform(ExecutionPolicy &&policy, dr::distributed_range auto &&in,
  ...
  for (auto &&[in_seg, out_seg] :
    ...
    if (in_seg.rank() == out_seg.rank()) {
      ...
      OutT *buffer =
          sycl::malloc_device<OutT>(seg_size, in_device, dr::sp::context());
...
auto transform(R &&in, Iter out, Fn &&fn) {
  return transform(dr::sp::par_unseq, std::forward<R>(in),
...
auto transform(Iter1 in_begin, Iter1 in_end, Iter2 out_end, Fn &&fn) {
  return transform(dr::sp::par_unseq, std::forward<Iter1>(in_begin),
