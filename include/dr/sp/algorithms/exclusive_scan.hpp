...
void exclusive_scan_impl_(ExecutionPolicy &&policy, R &&r, O &&o, U init,
  ...
  auto zipped_view = dr::sp::views::zip(r, o);
  ...
  if constexpr (std::is_same_v<std::remove_cvref_t<ExecutionPolicy>,
    ...
    sp::copy(d_inits, d_inits + inits.size(), inits.data() + 1);

    sycl::free(d_inits, sp::context());

    inits[0] = init;

    auto root = dr::sp::devices()[0];
    dr::sp::device_allocator<T> allocator(dr::sp::context(), root);
    dr::sp::vector<T, dr::sp::device_allocator<T>> partial_sums(
...
void exclusive_scan(R &&r, O &&o, T init, BinaryOp &&binary_op) {
  exclusive_scan_impl_(dr::sp::par_unseq, std::forward<R>(r),
...
void exclusive_scan(Iter first, Iter last, OutputIter d_first, T init,
                    BinaryOp &&binary_op) {
  exclusive_scan(dr::sp::par_unseq, first, last, d_first, init,
                 std::forward<BinaryOp>(binary_op));
}
