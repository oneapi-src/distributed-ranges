...
#include <dr/sp/algorithms/matrix/local_gemv.hpp>
#include <dr/sp/containers/duplicated_vector.hpp>
...
void flat_gemv(C &&c, dr::sp::sparse_matrix<T, I> &a, B &&b) {
  ...
  auto &&devices = dr::sp::devices();

  using b_scalar_type = rng::range_value_t<B>;

  using local_vector_type =
      dr::sp::device_vector<b_scalar_type,
                            dr::sp::device_allocator<b_scalar_type>>;
  ...
  for (std::size_t i = 0; i < devices.size(); i++) {
    dr::sp::device_allocator<T> allocator(dr::sp::context(), devices[i]);
    local_b.push_back(local_vector_type(b.size(), allocator, i));
  }

  for (auto &&l_b : local_b) {
    auto event =
        dr::sp::copy_async(b.begin(), b.end(), dr::ranges::local(l_b.begin()));
...
template <dr::distributed_range C, typename T, typename I,
          dr::distributed_range B>
void gemv(C &&c, dr::sp::sparse_matrix<T, I> &a, B &&b) {
  dr::sp::duplicated_vector<rng::range_value_t<B>> b_duplicated(b.size());
...
template <dr::distributed_range C, typename T, typename I,
          dr::distributed_range B>
void gemv_square(C &&c, dr::sp::sparse_matrix<T, I> &a, B &&b) {
  assert(a.shape()[0] == c.size());
  assert(a.shape()[1] == b.size());
  assert(a.grid_shape()[0] == c.segments().size());
...
template <dr::distributed_range C, typename T, typename I,
          dr::distributed_range B>
void gemv_square_copy(C &&c, dr::sp::sparse_matrix<T, I> &a, B &&b) {
  assert(a.shape()[0] == c.size());
  assert(a.shape()[1] == b.size());
  assert(a.grid_shape()[0] == c.segments().size());
  assert(a.grid_shape()[1] == b.segments().size());
  ...
  for (std::size_t i = 0; i < a.grid_shape()[0]; i++) {
    dr::sp::device_allocator<T> allocator(
        dr::sp::context(), devices[a.tile(dr::index<I>(i, 0)).rank()]);
  ...
  for (std::size_t i = 0; i < a.grid_shape()[0]; i++) {
    ...
    for (std::size_t k_ = 0; k_ < a.grid_shape()[1]; k_++) {
      ...
      auto ce =
          dr::sp::copy_async(q, b_segment.begin(), b_segment.end(), b_iter);
