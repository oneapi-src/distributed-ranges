...
template <typename T>
auto custom_gemm(sycl::queue &q, sp::dense_matrix_view<T> a,
                 sp::dense_matrix_view<T> b, sp::dense_matrix_view<T> c,
...
template <typename T>
auto mkl_gemm(sycl::queue &q, sp::dense_matrix_view<T> a,
              sp::dense_matrix_view<T> b, sp::dense_matrix_view<T> c,
              const std::vector<sycl::event> &dependencies = {}) {
  assert(c.shape()[0] == a.shape()[0]);
  assert(c.shape()[1] == b.shape()[1]);
...
template <typename T>
auto local_gemm(sycl::queue &q, sp::dense_matrix_view<T> a,
                sp::dense_matrix_view<T> b, sp::dense_matrix_view<T> c,
                const std::vector<sycl::event> &dependencies = {}) {
