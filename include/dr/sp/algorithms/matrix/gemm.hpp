...
#include <dr/sp/algorithms/matrix/local_gemm.hpp>
#include <dr/sp/containers/distributed_dense_matrix.hpp>
  ...
  for (std::size_t i = 0; i < c.grid_shape()[0]; i++) {
    for (std::size_t j = 0; j < c.grid_shape()[1]; j++) {
      ...
      threads.emplace_back([c_local, i, j, &a, &b, &communication, &compute] {
        ...
        dr::sp::device_allocator<T> gpu_allocator(q);
        dr::sp::buffered_allocator buffered_allocator(gpu_allocator,
                                                      buffer_size, 2);
        ...
        for (std::size_t k_ = 0; k_ < a.grid_shape()[1]; k_++) {
          ...
          dr::sp::dense_matrix_view a_local(a_tile);
          dr::sp::dense_matrix_view b_local(b_tile);
