...
void sort(R &&r, Compare comp = Compare()) {
  ...
  if (rng::size(segments) == 0) {
    ...
    auto &&local_policy =
        dr::sp::__detail::dpl_policy(dr::ranges::rank(segment));
    auto &&local_segment = dr::sp::__detail::local(segment);
