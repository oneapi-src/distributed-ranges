...
sycl::usm::alloc get_pointer_type(Iter iter) {
  return sycl::get_pointer_type(std::to_address(iter), sp::context());
}

template <typename T> sycl::usm::alloc get_pointer_type(sp::device_ptr<T> ptr) {
  return sycl::get_pointer_type(ptr.get_raw_pointer(), sp::context());
}

template <std::contiguous_iterator Iter>
sycl::device get_pointer_device(Iter iter) {
  return sycl::get_pointer_device(std::to_address(iter), sp::context());
}

template <typename T> sycl::device get_pointer_device(sp::device_ptr<T> ptr) {
  return sycl::get_pointer_device(ptr.get_raw_pointer(), sp::context());
}
