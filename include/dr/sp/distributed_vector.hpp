...
public:
  ...
  using const_segment_type =
      std::add_const_t<dr::sp::device_vector<T, Allocator>>;
  ...
  distributed_vector(std::size_t count = 0) {
    assert(dr::sp::devices().size() > 0);
    size_ = count;
    segment_size_ =
        (count + dr::sp::devices().size() - 1) / dr::sp::devices().size();
    capacity_ = segment_size_ * dr::sp::devices().size();

    std::size_t rank = 0;
    for (auto &&device : dr::sp::devices()) {
      segments_.emplace_back(segment_type(
          segment_size_, Allocator(dr::sp::context(), device), rank++));
    }
  }

  distributed_vector(std::size_t count, const T &value)
      : distributed_vector(count) {
    dr::sp::fill(*this, value);
  }

  distributed_vector(std::initializer_list<T> init)
      : distributed_vector(init.size()) {
    dr::sp::copy(rng::begin(init), rng::end(init), begin());
  }
  ...
  void resize(size_type count, const value_type &value) {
    ...
    dr::sp::copy(begin(), begin() + copy_size, other.begin());
    *this = std::move(other);
  }
