...
public:
  ...
  // using iterator = sparse_matrix_iterator<T, dr::sp::device_vector<T,
  ...
  auto copy_tile_async(key_type tile_index,
    ...
    auto v_e = dr::sp::copy_async(tile_view.values_data(),
                                  tile_view.values_data() + values.size(),
                                  values.data());

    auto c_e = dr::sp::copy_async(tile_view.colind_data(),
                                  tile_view.colind_data() + colind.size(),
                                  colind.data());

    auto r_e = dr::sp::copy_async(tile_view.rowptr_data(),
...
private:
  ...
  std::unique_ptr<dr::sp::matrix_partition> partition_;

  std::vector<dr::sp::device_vector<T, dr::sp::device_allocator<T>>> values_;
  std::vector<dr::sp::device_vector<I, dr::sp::device_allocator<I>>> rowptr_;
  std::vector<dr::sp::device_vector<I, dr::sp::device_allocator<I>>> colind_;
