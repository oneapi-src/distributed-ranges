...
#include <dr/sp/future.hpp>
#include <dr/sp/views/dense_matrix_view.hpp>
...
public:
  ...
  using iterator = distributed_dense_matrix_iterator<
      T, dr::sp::device_vector<T, dr::sp::device_allocator<T>>>;
  ...
  std::vector<dense_matrix_view<T, rng::iterator_t<dr::sp::device_vector<
                                       T, dr::sp::device_allocator<T>>>>>
  tiles() {
    std::vector<dense_matrix_view<T, rng::iterator_t<dr::sp::device_vector<
                                         T, dr::sp::device_allocator<T>>>>>
        views_;
  ...
  auto get_tile(key_type tile_index, const Allocator &alloc = Allocator{}) {
    ...
    sp::copy(remote_tile.data(), remote_tile.data() + tile_size,
             local_tile.data());
    return local_tile;
  }
...
private:
  ...
  std::vector<dr::sp::device_vector<T, dr::sp::device_allocator<T>>> tiles_;
};
