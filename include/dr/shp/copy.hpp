...
template <typename T>
requires(!std::is_const_v<T>) cl::sycl::event
...
template <typename T>
requires(!std::is_const_v<T>) device_ptr<T> copy(const T *first, const T *last,
...
template <typename T>
requires(!std::is_const_v<T>) cl::sycl::event
    copy_async(device_ptr<const T> first, device_ptr<const T> last,
               T *d_first) {
  cl::sycl::queue q;
...
template <typename T>
requires(!std::is_const_v<T>) T *copy(device_ptr<const T> first,
                                      device_ptr<const T> last, T *d_first) {
  copy_async(first, last, d_first).wait();
  return d_first + (last - first);
}
...
template <typename T>
requires(!std::is_const_v<T>) void fill(device_ptr<T> first, device_ptr<T> last,
                                        const T &value) {
  cl::sycl::queue q;
