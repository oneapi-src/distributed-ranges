...
auto inclusive_scan_no_init_async(ExecutionPolicy&& policy, Iter begin, Iter end,
                                  Iter d_begin) {
  return oneapi::dpl::experimental::inclusive_scan_async(
             std::forward<ExecutionPolicy>(policy), begin, end,
             d_begin);
}
...
void inclusive_scan(ExecutionPolicy&& policy, R&& r) {
  ...
  if constexpr (std::is_same_v<std::remove_cvref_t<ExecutionPolicy>,
    ...
    for (auto&& segment : lib::ranges::segments(r)) {
      ...
      if (dist >= 2) {
        auto future = inclusive_scan_no_init_async(local_policy, rng::begin(segment),
                                   rng::end(segment), rng::begin(segment));
    ...
    shp::vector<VT, shp::device_allocator<VT>> partial_sums(std::size_t(segments.size()), allocator);
...
/*
    ...
    */
    ...
    for (auto&& segment : segments) {
      ...
      oneapi::dpl::experimental::for_each_async(local_policy, rng::begin(segment), rng::end(segment),
              [=](auto&& x) {
                x += sum;
              }).get();
      idx++;
