...
public:
  ...
  iterator_adaptor(const const_accessor_type &accessor) requires(
      !std::is_same_v<accessor_type, const_accessor_type>)
      : accessor_(accessor) {}
  ...
  bool operator<(const_iterator other) const requires(
  ...
  reference operator[](difference_type offset) const requires(
      std::is_same_v<iterator_category, std::random_access_iterator_tag>) {
    return *(*this + offset);
  }

  iterator &operator+=(difference_type offset) noexcept requires(
      std::is_same_v<iterator_category, std::random_access_iterator_tag>) {
    accessor_ += offset;
    return *this;
  }
  ...
  iterator operator+(difference_type offset) const requires(
      std::is_same_v<iterator_category, std::random_access_iterator_tag>) {
    iterator other = *this;
    other += offset;
    return other;
  ...
  difference_type operator-(const_iterator other) const requires(
      std::is_same_v<iterator_category, std::random_access_iterator_tag>) {
    return accessor_ - other.accessor_;
  }

  iterator &operator++() noexcept requires(
      std::is_same_v<iterator_category, std::random_access_iterator_tag>) {
    *this += 1;
    return *this;
  }
  ...
  iterator operator--(int) noexcept requires(
      std::is_same_v<iterator_category, std::random_access_iterator_tag> ||
      std::is_same_v<iterator_category, std::bidirectional_iterator_tag>) {
    iterator other = *this;
    --(*this);
  ...
  friend iterator operator+(difference_type n, iterator iter) requires(
      std::is_same_v<iterator_category, std::random_access_iterator_tag>) {
    return iter + n;
  }
