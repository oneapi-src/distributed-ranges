...
public:
  ...
  void broadcast_data(std::size_t height, std::size_t width, std::size_t root,
                      T **root_data, dr::communicator comm) {
    if (_data != nullptr) {
      destroy_data();
    }
    ...
    if (comm.rank() == root) {
      for (auto i = 0; i < width; i++) {
        if (use_sycl()) {
          __detail::sycl_copy(root_data[i], root_data[i] + height,
                              _data + height * i);
        } else {
  ...
  void broadcast_data(std::size_t height, std::size_t width, std::size_t root,
                      R root_data, dr::communicator comm) {
    if (_data != nullptr) {
      destroy_data();
    }
    _data_size = height * width;
    ...
    if (comm.rank() == root) {
      if (use_sycl()) {
        __detail::sycl_copy(std::to_address(root_data.begin()),
                            std::to_address(root_data.end()), _data);
      } else {
  ...
  T *operator[](std::size_t index) { return _data + _height * index; }
