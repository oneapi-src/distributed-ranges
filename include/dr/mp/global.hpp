...
// File with initialization/cleanup functions, global objects created during
// initialization and helper functions using these objects. For general helper
// functions see sycl_support.hpp and common_support.hpp
...
inline auto gcontext() {
  assert(global_context_ && "Call mp::init() after MPI_Init()");
...
inline void init() {
  ...
  assert(__detail::global_context_ == nullptr &&
         "Do not call mp::init() more than once");
