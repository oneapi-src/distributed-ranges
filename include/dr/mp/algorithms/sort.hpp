...
void sort(R &r, Compare &&comp = Compare()) {
  ...
  if (_comm_size == 1) {
     ...
     */
    DRLOG("mp::sort() - local sort on node 0");

    std::vector<valT> vec_recvdata(rng::size(r));
    dr::mp::copy(0, r, rng::begin(vec_recvdata));

    if (_comm_rank == 0) {
      rng::sort(vec_recvdata, comp);
    }
    dr::mp::barrier();
    dr::mp::copy(0, vec_recvdata, rng::begin(r));

  } else {
    DRLOG("mp::sort() - distributed sort");
