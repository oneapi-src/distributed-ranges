...
TYPED_TEST(Take, lessThanSize) {
  ...
  auto local = rng::views::take(ops.vec, 6);
  auto dist = shp::views::take(ops.dist_vec, 6);
  EXPECT_TRUE(check_view(local, dist));
}
...
TYPED_TEST(Take, sameSize) {
  ...
  auto dist = shp::views::take(ops.dist_vec, 10);
  EXPECT_TRUE(check_view(local, dist));
}
...
TYPED_TEST(Take, moreSize) {
  ...
  auto local = rng::views::take(ops.vec, 10);
  auto dist = shp::views::take(ops.dist_vec, 12);
  EXPECT_TRUE(check_view(local, dist));
}
...
TYPED_TEST(Take, zero) {
  ...
  auto dist = shp::views::take(ops.dist_vec, 0);
  EXPECT_TRUE(check_view(local, dist));
}
...
TYPED_TEST(Take, one) {
  ...
  auto dist = shp::views::take(ops.dist_vec, 1);
  EXPECT_TRUE(check_view(local, dist));
}
...
TYPED_TEST(Take, large) {
  ...
  auto dist = shp::views::take(ops.dist_vec, 1000);
  EXPECT_TRUE(check_view(local, dist));
}
