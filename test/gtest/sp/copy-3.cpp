...
TYPED_TEST(CopyTest, tests_from_this_file_run_on_3_devices) {
  ...
  EXPECT_EQ(rng::size(dr::sp::devices()), 3);
}
...
TYPED_TEST(CopyTest, dist2local_wholesegment) {
  ...
  auto ret_it = dr::sp::copy(rng::begin(dist_vec) + 4, rng::begin(dist_vec) + 8,
                             rng::begin(local_vec));
  EXPECT_TRUE(
      equal_gtest(local_vec, typename TestFixture::LocalVec{5, 6, 7, 8}));
...
TYPED_TEST(CopyTest, local2dist_wholesegment) {
  ...
  EXPECT_TRUE(equal_gtest(dist_vec,
                          typename TestFixture::LocalVec{1, 2, 3, 4, 50, 60, 70,
                                                         80, 9, 10, 11, 12}));
  EXPECT_EQ(*ret_it, 9);
}
