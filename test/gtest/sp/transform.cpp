...
TYPED_TEST(TransformTest, whole_aligned) {
  ...
  auto r = dr::sp::transform(dr::sp::par_unseq, a, rng::begin(b),
...
TYPED_TEST(TransformTest, part_aligned) {
  ...
  auto [r_in, r_out] = dr::sp::transform(
      dr::sp::par_unseq, rng::subrange(++rng::begin(a), --rng::end(a)),
...
TYPED_TEST(TransformTest, zip_merge) {
  ...
  // dr::sp::distributed_vector<int> res(5);

  // auto min = [=](auto &&pair) {
  //   return std::min(pair.first, pair.second);
  // };
  // auto zipped_view = dr::sp::views::zip(a, b);
  // dr::sp::transform(dr::sp::par_unseq, zipped_view, res.begin(), min);
...
TYPED_TEST(TransformTest, inplace_whole) {
  ...
  auto [r_in, r_out] = dr::sp::transform(dr::sp::par_unseq, a, rng::begin(a),
                                         TestFixture::add_10_func);
...
TYPED_TEST(TransformTest, large_aligned_whole) {
  ...
  dr::sp::transform(dr::sp::par_unseq, a, rng::begin(b),
                    TestFixture::add_10_func);
