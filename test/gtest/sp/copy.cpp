...
TYPED_TEST(CopyTest, dist2local_async) {
  ...
  dr::sp::copy_async(rng::begin(dist_vec), rng::end(dist_vec),
                     rng::begin(local_vec))
      .wait();
  EXPECT_TRUE(
      equal_gtest(local_vec, typename TestFixture::LocalVec{1, 2, 3, 4, 5}));
}
...
TYPED_TEST(CopyTest, local2dist_async) {
  ...
  dr::sp::copy_async(rng::begin(local_vec), rng::end(local_vec),
                     rng::begin(dist_vec))
      .wait();
  EXPECT_TRUE(
      equal_gtest(dist_vec, typename TestFixture::LocalVec{1, 2, 3, 4, 5}));
}

TYPED_TEST(CopyTest, dist2local_sync) {
  const typename TestFixture::DistVec dist_vec = {1, 2, 3, 4, 5};
  typename TestFixture::LocalVec local_vec = {0, 0, 0, 0, 0, 9};
  auto ret_it = dr::sp::copy(rng::begin(dist_vec), rng::end(dist_vec),
                             rng::begin(local_vec));
  EXPECT_TRUE(
...
TYPED_TEST(CopyTest, local2dist_sync) {
  ...
  auto ret_it = dr::sp::copy(rng::begin(local_vec), rng::end(local_vec),
                             rng::begin(dist_vec));
  EXPECT_TRUE(
...
TYPED_TEST(CopyTest, dist2local_range_sync) {
  const typename TestFixture::DistVec dist_vec = {1, 2, 3, 4, 5};
  typename TestFixture::LocalVec local_vec = {0, 0, 0, 0, 0, 9};
  auto ret_it = dr::sp::copy(dist_vec, rng::begin(local_vec));
  EXPECT_TRUE(
      equal_gtest(local_vec, typename TestFixture::LocalVec{1, 2, 3, 4, 5, 9}));
  EXPECT_EQ(*ret_it, 9);
}
...
TYPED_TEST(CopyTest, local2dist_range_sync) {
  const typename TestFixture::LocalVec local_vec = {1, 2, 3, 4, 5};
  typename TestFixture::DistVec dist_vec = {0, 0, 0, 0, 0, 9};
  auto ret_it = dr::sp::copy(local_vec, rng::begin(dist_vec));
  EXPECT_TRUE(
      equal_gtest(dist_vec, typename TestFixture::LocalVec{1, 2, 3, 4, 5, 9}));
  EXPECT_EQ(*ret_it, 9);
}
...
TYPED_TEST(CopyTest, dist2local_async_can_interleave) {
  ...
  auto event_1 =
      dr::sp::copy_async(rng::begin(dist_vec) + 0, rng::begin(dist_vec) + 4,
                         rng::begin(local_vec) + 0);
  auto event_2 =
      dr::sp::copy_async(rng::begin(dist_vec) + 1, rng::begin(dist_vec) + 5,
                         rng::begin(local_vec) + 4);
  event_1.wait();
  event_2.wait();
  EXPECT_TRUE(equal_gtest(
      local_vec, typename TestFixture::LocalVec{1, 2, 3, 4, 2, 3, 4, 5}));
}
...
TYPED_TEST(CopyTest, dist2local_sliced_bothSides) {
  ...
  dr::sp::copy(rng::begin(dist_vec) + 1, rng::end(dist_vec) - 1,
               rng::begin(local_vec));
  EXPECT_TRUE(equal_gtest(
...
TYPED_TEST(CopyTest, dist2local_sliced_left) {
  ...
  dr::sp::copy(rng::begin(dist_vec) + 1, rng::end(dist_vec),
               rng::begin(local_vec));
  EXPECT_TRUE(equal_gtest(local_vec, typename TestFixture::LocalVec{
...
TYPED_TEST(CopyTest, dist2local_sliced_right) {
  ...
  dr::sp::copy(rng::begin(dist_vec), rng::end(dist_vec) - 1,
               rng::begin(local_vec));
  EXPECT_TRUE(equal_gtest(
...
TYPED_TEST(CopyTest, local2dist_sliced_bothSides) {
  ...
  dr::sp::copy(rng::begin(local_vec), rng::end(local_vec),
