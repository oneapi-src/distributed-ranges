...
TEST(WaveKernel, DeDx) {
  ...
  // auto e_view = dr::mp::views::submdspan(e.view(), {1, 0},
  //                                         {e.mdspan().extent(0)-1,
  //                                          e.mdspan().extent(1)});
  auto e_view = dr::mp::views::submdspan(e.view(), start, end);
  auto dedx_view = dr::mp::views::submdspan(dedx.view(), start, end);
  dr::mp::stencil_for_each(rhs_dedx, e_view, dedx_view);
...
TEST(WaveKernel, DeDy) {
  ...
  std::array<std::size_t, 2> end{
      ...
      static_cast<std::size_t>(e.mdspan().extent(1))};
  auto e_view = dr::mp::views::submdspan(e.view(), start, end);
  auto dedy_view = dr::mp::views::submdspan(dedy.view(), start, end);
  dr::mp::stencil_for_each(rhs_dedy, e_view, dedy_view);
...
TEST(WaveKernel, DuDx) {
  ...
  auto dudx_view = dr::mp::views::submdspan(dudx.view(), start, end);
  dr::mp::stencil_for_each(rhs_dudx, u_view, dudx_view);
...
TEST(WaveKernel, DvDy) {
  ...
  auto dvdy_view = dr::mp::views::submdspan(dvdy.view(), start, end);
  dr::mp::stencil_for_each(rhs_dvdy, v_view, dvdy_view);
...
TEST(WaveKernel, Divergence) {
  ...
  auto u_view = dr::mp::views::submdspan(u.view(), start, end);
  auto v_view = dr::mp::views::submdspan(v.view(), start, end);
  auto divuv_view = dr::mp::views::submdspan(divuv.view(), start, end);
  dr::mp::stencil_for_each(rhs_div, u_view, v_view, divuv_view);
