...
auto dot_product_distributed(dr::distributed_range auto &&x,
  ...
  auto z = mp::views::zip(x, y) | mp::views::transform([](auto &&elem) {
             auto &&[a, b] = elem;
             return a * b;
           });
...
auto dot_product_onedpl(sycl::queue q, X &&x, Y &&y) {
  auto z = rng::views::zip(x, y) | mp::views::transform([](auto &&elem) {
             auto &&[a, b] = elem;
             return a * b;
           });
...
void stats(auto &durations, auto &sum, auto v_serial, auto &x_local,
           auto &y_local) {
  fmt::print("MP executing on {} ranks:\n", comm_size);
...
int main(int argc, char **argv) {
  ...
  cxxopts::Options options_spec(argv[0], "mp dot product benchmark");
  // clang-format off
  ...
  std::string device_string =
      mp::sycl_queue().get_device().get_info<sycl::info::device::name>();

  for (std::size_t i = 0; i < size; i++) {
    if (rank == i) {
      fmt::print("Rank {} executing on device {} on node {}\n", rank,
                 device_string, mp::hostname());
    }
    fflush(stdout);
    mp::barrier();
    fflush(stdout);
  ...
  int error = 0;
  {
    mp::distributed_vector<T> x(n);
    mp::distributed_vector<T> y(n);

    mp::copy(0, x_local, x.begin());
    mp::copy(0, y_local, y.begin());

    std::vector<double> durations;
    durations.reserve(n_iterations);

    // Execute on all devices with MP:
    T sum = 0;
    for (std::size_t i = 0; i < n_iterations; i++) {
      auto begin = std::chrono::high_resolution_clock::now();
