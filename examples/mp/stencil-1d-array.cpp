...
int stencil() {
  ...
  dr::mp::distributed_vector<Row> a(rows, dist), b(rows, dist);
  dr::mp::for_each(a, [](auto &&row) { rng::iota(row, 100); });
  dr::mp::for_each(b, [](auto &&row) { rng::fill(row, 0); });

  auto in = rng::subrange(a.begin() + 1, a.end() - 1);
  auto out = rng::subrange(b.begin() + 1, b.end() - 1);
  for (std::size_t s = 0; s < steps; s++) {
    dr::mp::halo(in).exchange();
    dr::mp::for_each(dr::mp::views::zip(in, out), stencil_op);
    std::swap(in, out);
  }
