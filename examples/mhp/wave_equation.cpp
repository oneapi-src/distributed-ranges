...
void rhs(dr::mhp::distributed_dense_matrix<T> &u,
         ...
         dr::mhp::distributed_dense_matrix<T> &dedt,
  ...
  // auto u_interior = dr::mhp::subrange(u, {1, u.shape()[0] - 1}, {0, u.shape()[1]});
  // v without boundary values (nx, ny-1)
  // auto v_interior = dr::mhp::subrange(v, {0, v.shape()[0]}, {1, v.shape()[1] - 1});
  ...
  auto dudx_view = dr::mhp::subrange(dudx, {0, dudx.shape()[0]}, {0, dudx.shape()[1]});
  auto dvdy_view = dr::mhp::subrange(dvdy, {0, dvdy.shape()[0]}, {0, dvdy.shape()[1]});

  auto dudt_interior = dr::mhp::subrange(dudt, {1, u.shape()[0] - 1}, {0, dudt.shape()[1]});
  auto dvdt_interior = dr::mhp::subrange(dvdt, {0, u.shape()[0]}, {1, dvdt.shape()[1] - 1});
  ...
  // dr::mhp::transform(dr::mhp::views::zip(upx, vpy), dedt.begin(), rhs_divhuv);
...
int main(int argc, char *argv[]) {
  ...
  if (comm_rank == 0) {
    ...
    std::cout << "Velocity  DOFs: " << (nx+1)*ny + nx*(ny+1) << std::endl;
  ...
  size_t nt = static_cast<int>(ceil(t_end / dt));
  if (comm_rank == 0) {
  ...
  auto rk_update2 = [](auto ops) {
    return 0.75*std::get<0>(ops) + 0.25*(std::get<1>(ops) + std::get<2>(ops));
  };
  auto rk_update3 = [](auto ops) {
    return 1.0/3.0*std::get<0>(ops) + 2.0/3.0*(std::get<1>(ops) + std::get<2>(ops));
  };
  ...
  for (size_t i = 0; i < nt+1; i++) {
    ...
    if (t >= next_t_export - 1e-8) {
        ...
        double total_v = (dr::mhp::reduce(e, static_cast<T>(0), std::plus{}, 0) + h) * dx * dy;
        if (i == 0) {
          initial_v = total_v;
        }
    ...
    dr::mhp::transform(dr::mhp::views::zip(u, u1, dudt), u2.begin(), rk_update2);
    dr::mhp::transform(dr::mhp::views::zip(v, v1, dvdt), v2.begin(), rk_update2);
    dr::mhp::transform(dr::mhp::views::zip(e, e1, dedt), e2.begin(), rk_update2);
    ...
    dr::mhp::transform(dr::mhp::views::zip(u, u2, dudt), u3.begin(), rk_update3);
    dr::mhp::transform(dr::mhp::views::zip(v, v2, dvdt), v3.begin(), rk_update3);
    dr::mhp::transform(dr::mhp::views::zip(e, e2, dedt), e3.begin(), rk_update3);
  ...
  dr::mhp::transform(dr::mhp::views::zip(e, e_exact), error.begin(), error_kernel);
  double err_L2 = dr::mhp::reduce(error, static_cast<T>(0), std::plus{}) * dx * dy / lx / ly;
  ...
  if (nx < 128 or ny < 128) {
    ...
    if (fabs(rel_err) > rel_tolerance) {
      if (comm_rank == 0) {
        std::cout << "ERROR: L2 error deviates from reference value: " << expected_L2 << ", relative error: " << rel_err << std::endl;
      }
      return 1;
    }
  } else {
