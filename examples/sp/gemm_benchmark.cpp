...
auto serial_test(std::size_t m, std::size_t n, std::size_t k) {
  ...
  sp::dense_matrix_view<T> a(a_d, {m, k}, k, 0);
  sp::dense_matrix_view<T> b(b_d, {k, n}, n, 0);
  sp::dense_matrix_view<T> c(c_d, {m, n}, n, 0);
...
template <rng::forward_range R> auto values_view(R &&m) {
  return m | sp::views::transform([](auto &&e) {
           auto &&[_, v] = e;
           return v;
         });
}

template <typename T> auto sum_matrix(sp::distributed_dense_matrix<T> &m) {
  auto view = values_view(m);
  auto &&segments = view.segments();

  return sp::reduce(view, T(0));
}

template <typename T, typename U>
void assign(sp::distributed_dense_matrix<T> &m, const U &value) {
...
int main(int argc, char **argv) {
  ...
  auto partitions = sp::partition_matmul(m, n, k);

  using T = float;

  dr::sp::distributed_dense_matrix<T> a({m, k}, partitions[0]);
  dr::sp::distributed_dense_matrix<T> b({k, n}, partitions[1]);
  dr::sp::distributed_dense_matrix<T> c({m, n}, partitions[2]);

  dr::sp::distributed_dense_matrix<T> c_ref({m, n}, partitions[2]);

  dr::sp::distributed_dense_matrix<T> c_diff({m, n}, partitions[2]);

  auto shape = a.shape();
  dr::sp::for_each(dr::sp::par_unseq, a, [=](auto &&entry) {
    auto &&[idx, v] = entry;
    v = static_cast<T>(idx[0] + idx[1]) / shape[0];
  });

  shape = b.shape();
  dr::sp::for_each(dr::sp::par_unseq, b, [=](auto &&entry) {
    auto &&[idx, v] = entry;
    v = static_cast<T>(idx[0] + idx[1]) / shape[0];
  });
  ...
  for (std::size_t i = 0; i < n_iterations; i++) {
    ...
    auto sub_view = sp::views::zip(values_view(c), values_view(c_ref)) |
                    sp::views::transform([](auto &&e) {
                      auto &&[value, ref] = e;

                      return std::abs(value - ref);
                    });

    T diff_sum = sp::reduce(sp::par_unseq, sub_view, T(0));
