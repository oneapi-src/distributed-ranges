...
int main(int argc, char **argv) {
  auto devices = dr::sp::get_duplicated_devices(sycl::default_selector_v, 16);
  dr::sp::init(devices);
  ...
  dr::sp::distributed_dense_matrix<float> a({m, k}, partitions[0]);
  dr::sp::distributed_dense_matrix<float> b({k, n}, partitions[1]);
  dr::sp::distributed_dense_matrix<float> c({m, n}, partitions[2]);
  ...
  shape = c.shape();
  dr::sp::for_each(dr::sp::par_unseq, c, [=](auto &&entry) {
    auto &&[idx, v] = entry;
    v = 0;
  });
