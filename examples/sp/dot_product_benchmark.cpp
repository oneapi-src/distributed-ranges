...
auto dot_product_distributed(X &&x, Y &&y) {
  auto z = dr::sp::views::zip(x, y) | dr::views::transform([](auto &&elem) {
             auto &&[a, b] = elem;
             return a * b;
           });

  return dr::sp::reduce(dr::sp::par_unseq, z, rng::range_value_t<X>(0),
                        std::plus());
}
...
auto dot_product_sequential(X &&x, Y &&y) {
  auto z = dr::sp::views::zip(x, y) | rng::views::transform([](auto &&elem) {
             auto &&[a, b] = elem;
             return a * b;
           });
...
int main(int argc, char **argv) {
...

  {
    ...
    dr::sp::distributed_vector<T> x(n, 1);
    dr::sp::distributed_vector<T> y(n, 1);

    // Execute on all devices with SP:
    fmt::print("Execute with SP...\n");
  ...
  sycl::queue q_i(dr::sp::context(), dr::sp::devices()[0],
