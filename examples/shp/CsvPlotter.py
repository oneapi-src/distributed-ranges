...
def plot_time_of_devices(csv_file="examples/shp/sycl_bench_output.csv", output_plots="examples/shp/plots"):
    ...
    for i, vec_size in enumerate(vec):
        ...
        y = data.values()
        plt.plot(x, y, 'ro')
        plt.title(f"Median out of a device number for vector size - {vec_size}")
