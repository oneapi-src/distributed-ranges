...
struct Checker {
  ...
  void check_array(rng::forward_range auto &&actual) {
    ...
    dr::mp::copy(0, actual, local.begin());
    auto s = default_shape();
    auto rows = s[0];
    auto cols = s[1];
...
static void Stencil2D_ForeachStdArray_DR(benchmark::State &state) {
  ...
  dr::mp::distributed_vector<Row> a(rows, dist);
  dr::mp::distributed_vector<Row> b(rows, dist);
  Stats stats(state, sizeof(T) * a.size(), sizeof(T) * b.size());
...
static void Stencil2D_Segmented_DR(benchmark::State &state) {
  ...
  auto dist = dr::mp::distribution().halo(cols).granularity(cols);
  dr::mp::distributed_vector<T> a(rows * cols, init_val, dist);
  dr::mp::distributed_vector<T> b(rows * cols, init_val, dist);
  Stats stats(state, sizeof(T) * a.size(), sizeof(T) * b.size());

  Checker checker;
  auto in =
      dr::mp::local_segment(rng::subrange(a.begin() + cols, a.end() - cols));
  auto out =
      dr::mp::local_segment(rng::subrange(b.begin() + cols, b.end() - cols));
  ...
  for (auto _ : state) {
    for (std::size_t s = 0; s < stencil_steps; s++) {
      stats.rep();
      dr::mp::halo(stencil_steps % 2 ? b : a).exchange();
