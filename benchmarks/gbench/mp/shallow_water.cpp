...
void compute_total_depth(Array &e, Array &h, Array &H_at_f) {
  ...
  { // interior part
    ...
    auto Hf_view = dr::mp::views::submdspan(H_at_f.view(), start, end);
    dr::mp::stencil_for_each(kernel, e_view, h_view, Hf_view);
  }
  { // top row
    auto kernel = [](auto tuple) {
      auto [e, h, out] = tuple;
...
void compute_aux_fields(Array &u, Array &v, Array &e, Array &hu, Array &hv,
  ...
  { // dudy
    ...
    auto dudy_view = dr::mp::views::submdspan(dudy.view(), start, end);
    dr::mp::stencil_for_each(kernel, u_view, dudy_view);
  }
  ...
  { // dvdx
    ...
    auto dvdx_view = dr::mp::views::submdspan(dvdx.view(), start, end);
    dr::mp::stencil_for_each(kernel, v_view, dvdx_view);
  }
  ...
  { // vorticity
    ...
    dr::mp::stencil_for_each(kernel, dudy, dvdx, H_at_f, q);
  }
  dr::mp::halo(q).exchange_begin();
  dr::mp::halo(q).exchange_finalize();
  ...
  { // q advection
    ...
    auto q_view = dr::mp::views::submdspan(q.view(), start, end);
    auto qa_view = dr::mp::views::submdspan(qa.view(), start, end);
    auto qb_view = dr::mp::views::submdspan(qb.view(), start, end);
  ...
  dr::mp::halo(qg).exchange_begin();
  dr::mp::halo(qd).exchange_begin();
  ...
  { // hv
    ...
    auto e_view = dr::mp::views::submdspan(e.view(), start, end);
    auto h_view = dr::mp::views::submdspan(h.view(), start, end);
    auto hv_view = dr::mp::views::submdspan(hv.view(), start, end);
    dr::mp::stencil_for_each(kernel, v_view, e_view, h_view, hv_view);
  }
  dr::mp::halo(hv).exchange_begin();
  ...
  { // hu
    ...
    auto e_view = dr::mp::views::submdspan(e.view(), start, end);
    auto h_view = dr::mp::views::submdspan(h.view(), start, end);
    auto hu_view = dr::mp::views::submdspan(hu.view(), start, end);
    dr::mp::stencil_for_each(kernel, u_view, e_view, h_view, hu_view);
  }
  dr::mp::halo(hu).exchange_begin();

  dr::mp::halo(qa).exchange_finalize();
  dr::mp::halo(qb).exchange_finalize();
  dr::mp::halo(qg).exchange_finalize();
  dr::mp::halo(qd).exchange_finalize();
  dr::mp::halo(hv).exchange_finalize();
  dr::mp::halo(hu).exchange_finalize();
}
...
void rhs(Array &u, Array &v, Array &e, Array &hu, Array &hv, Array &dudy,
  ...
  { // dedt
    ...
    dr::mp::stencil_for_each(kernel, hu_view, hv_view, dedt_view);
  }
};
...
void stage1(Array &u, Array &v, Array &e, Array &hu, Array &hv, Array &dudy,
  ...
  { // e update
    ...
    auto hu_view = dr::mp::views::submdspan(hu.view(), start, end);
    auto hv_view = dr::mp::views::submdspan(hv.view(), start, end);
    auto e1_view = dr::mp::views::submdspan(e1.view(), start, end);
    dr::mp::stencil_for_each(kernel, e_view, hu_view, hv_view, e1_view);
  }
...
void stage3(Array &u, Array &v, Array &e, Array &hu, Array &hv, Array &dudy,
  ...
  { // e update
    ...
    auto hv_view = dr::mp::views::submdspan(hv.view(), start, end);
    auto e_view = dr::mp::views::submdspan(e.view(), start, end);
    dr::mp::stencil_for_each(kernel, e2_view, hu_view, hv_view, e_view);
  }
...
int run(
  ...
  dr::mp::for_each(h_init_op, h);
  dr::mp::halo(h).exchange();
  ...
  double pe_offset;
  {
    ...
    dr::mp::transform(h, h2.begin(), square);
    auto h2mean = dr::mp::reduce(h2, static_cast<T>(0), std::plus{}) / nx / ny;
  ...
  double h_max = dr::mp::reduce(h, static_cast<T>(0), max);
  ...
  for (std::size_t i = 0; i < nt + 1; i++) {
    ...
    if (t >= next_t_export - 1e-8) {
      ...
      double q_max = dr::mp::reduce(q, static_cast<T>(0), max);
      ...
      dr::mp::transform(dr::mp::views::zip(e, h), pe.begin(), pe_kernel);
      double total_pe = ((dr::mp::reduce(pe, static_cast<T>(0), std::plus{})) +
      ...
      dr::mp::transform(dr::mp::views::zip(e, h), H.begin(), add);
      double total_vol = (dr::mp::reduce(H, static_cast<T>(0), std::plus{})) *
                         grid.dx * grid.dy;
      if (i == 0) {
