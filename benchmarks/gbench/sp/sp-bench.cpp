...
// sp always uses device memory, but it is optional for mp
...
int main(int argc, char *argv[]) {
  ...
  cxxopts::Options options_spec(argv[0], "DR SP tests");
  ...
  options_spec.add_options()
    ...
    ("device-memory", "Ignored because sp always uses device memory")
  ...
  auto available_devices = dr::sp::get_numa_devices(sycl::default_selector_v);
  if (ranks == 0) {
