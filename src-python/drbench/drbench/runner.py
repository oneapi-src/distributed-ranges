...
class Runner:
    ...
    def __shp(self, analysis_case: AnalysisCase):
        return analysis_case.mode in [
            AnalysisMode.SHP_CPU_SYCL,
            AnalysisMode.SHP_GPU_SYCL,
        ]
    ...
    def __out_filename(self, case: AnalysisCase):
        ...
        rank = "" if self.__shp(case) else ".rankNNN"
        return f"{prefix}{rank}.{case.mode}.n{case.nprocs}.s{case.size}.json"

    def __run_mhp_analysis(self, params, case: AnalysisCase):
        if case.mode == AnalysisMode.MHP_CPU_SYCL:
            env = "ONEAPI_DEVICE_SELECTOR=opencl:cpu"
            params.append("--sycl")
            params.append("--context device:cpu")
        elif case.mode == AnalysisMode.MHP_GPU_SYCL:
            env = "ONEAPI_DEVICE_SELECTOR='level_zero:gpu;ext_oneapi_cuda:gpu'"
            params.append("--sycl")
            params.append("--context device:gpu")
        elif case.mode == AnalysisMode.MHP_CPU_DIRECT:
            env = (
        ...
        mpirun_params = []
        mpirun_params.append(f"-n {str(case.nprocs)}")
    ...
    def __run_shp_analysis(self, params, case: AnalysisCase):
        if case.mode == AnalysisMode.SHP_CPU_SYCL:
            env = "ONEAPI_DEVICE_SELECTOR=opencl:cpu"
            params.append("--context device:cpu")
        elif case.mode == AnalysisMode.SHP_GPU_SYCL:
            env = (
                "ONEAPI_DEVICE_SELECTOR='level_zero:gpu;ext_oneapi_cuda:gpu'"
                " KMP_AFFINITY=compact"
            )
            params.append("--context device:gpu")
        params.append(f"--num-devices {case.nprocs}")
        self.__execute(
    ...
    def run_one_analysis(self, analysis_case: AnalysisCase):
        ...
        outfname = self.__out_filename(analysis_case)
        ...
        if self.__shp(analysis_case):
            self.__run_shp_analysis(params, analysis_case)
        else:
            self.__run_mhp_analysis(params, analysis_case)
